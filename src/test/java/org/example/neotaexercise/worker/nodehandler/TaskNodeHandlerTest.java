...
class TaskNodeHandlerTest {
    ...
    private static final TaskNodeHandler TESTED = new TaskNodeHandler();

    @Test
    void support_test() {
        assertTrue(TESTED.support(WorkflowDefinitionDto.Node.Type.TASK_NODE));
        assertFalse(TESTED.support(WorkflowDefinitionDto.Node.Type.START_NODE));
    }
    ...
    void process_in_chain_test() {
        ...
        final var result = TESTED.process(new NodeHandler.ProcessContext(
            new PrintStream(out),
            new WorkflowDefinitionDto.Node("node1", WorkflowDefinitionDto.Node.Type.TASK_NODE, "Cool name"),
            Optional.empty(),
            new WorkflowDefinition(null, null, Map.of("node1", "node2"), emptyMap(), emptyMap())
        ));

        assertEquals(Optional.of("node2"), result);
        assertEquals("Cool name completed\n", out.toString());
    }
    ...
    void process_unknown_command_test() {
        ...
        assertEquals("Command START_PROCESS not supported for node type TASK_NODE\n", out.toString());
    }
}